...
def train_test_split():
    ...
    train_test_data_df = pd.DataFrame(train_test_data, columns=['user', 'item', 'score'])
    train_test_data_df.to_csv('./datasets/valid.csv', index=False)
    train_train_data_df = pd.DataFrame(train_train_data, columns=['user', 'item', 'score'])
    ...
    static_analyse(train_test_data_df['user'].nunique(),
                   train_test_data_df['item'].nunique(),
                   len(train_test_data_df))
    print("train train data")
    static_analyse(train_train_data_df['user'].nunique(),
...
def process_item_attribute():
    ...
    item_attributes_df = pd.DataFrame(item_attributes, columns=['item', 'attribute1', 'attribute2'])
...
def calculate_data_bias():
    ...
    deviation_of_user = train_train_data_df.groupby('user')['score'].mean() - overall_mean
    deviation_of_item = train_train_data_df.groupby('item')['score'].mean() - overall_mean
