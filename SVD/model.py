...
class SVDModel(object):
    def __init__(self):
        ...
        self.P = [
            [random.random() / sqrt_factors for i in range(0, self.factors)]
    ...
    def loadTrainSet(self):
        ...
        for train_line in trainSet:
            ...
            uid = self.user_dict[
                int(user_id)
            ]  # Find the user's Id in this program based on the actual user id
            iid = self.item_dict[
                int(item_id)
            ]  # Find the item's Id in this program based on the actual item id
            user_rate = float(rate_str)
            self.lil_matrix.append(
                (uid, iid, user_rate)
            )  # Store in sprase_matrix as (program's user id, program's item id, user rating)
    ...
    def train(self):
        ...
        for epoch in range(0, self.epochs):
            # for epoch in tqdm(range(0, self.epochs), desc="Training epochs"):
            rmse = 0.0
            ...
            for u, i, r in self.lil_matrix:
                # 计算预测分值
                rp = (
                    self.overall_train_mean_rating
                    + self.user_bias[u]
                    + self.item_bias[i]
                    + self.dot(u, i)
                )
                ...
                self.user_bias[u] += self.learning_rate * (
                    diff - self.LambdaUB * self.user_bias[u]
                )
                self.item_bias[i] += self.learning_rate * (
                    diff - self.LambdaIB * self.item_bias[i]
                )
                for k in range(0, self.factors):
                    ...
                    self.P[u][k] += self.learning_rate * (
                        diff * q_i_k - self.LambdaP * p_u_k
                    )
                    self.Q[i][k] += self.learning_rate * (
                        diff * p_u_k - self.LambdaQ * q_i_k
                    )
                rmse += diff**2
    ...
    def loadItemAttributeDataset(self):
        ...
        for user, item, rate, diff in self.lil_matrix_diff:
            ...
            self.user_item_attrs[user].append(
                (item, diff, self.item_attr[item][0], self.item_attr[item][1])
            )
    ...
    def evaluate(self):
        ...
        with open(valid_set, "r") as f, open(
            RESULT_FOLDER + "eval_result.csv", "w"
        ) as resultf:
            count = 0
            for line in f.readlines():
                if line == "":
                ...
                num_test_data += 1
        rmse, rmse_with_IA = math.sqrt(rmse / num_test_data), math.sqrt(
            rmse_with_IA / num_test_data
        )
        print(f"验证集上的RMSE为: {rmse}")
