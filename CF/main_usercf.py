...
if __name__ == "__main__":
    ...
    if is_retrain == "y":
        model = UserCF(
            "../datasets/train.csv", "../datasets/valid.csv", model_directory
        )
        ...
        if not model.is_build:
            model.build("../datasets/train.csv")
            model.train()
            save_model(model, model_path)
            ...
            model.test("../datasets/valid.csv")
            save_model(model, tested_model_path)
        else:
            assert (
                False
            ), "Unexpected state: Model is already built, trained, and tested."
