...
class CollaborativeFiltering:
    ...
    def calculate_similarity(self, user, item_i):
        ...
        for item_j in self.user_item_train_data[user].keys():
            ...
            if similarity_dict[item_j] is None:
                bias_j = (
                    self.bias["deviation_of_item"].get(item_j, 0)
                    + self.bias["overall_mean"]
                )

                if (
                    self.item_attributes[item_i][2] == -1
                    or self.item_attributes[item_j][2] == -1
                ):
                    attribute_similarity = 0
                else:
                    attribute_similarity = (
                        self.item_attributes[item_i][0]
                        * self.item_attributes[item_j][0]
                ...
                norm_j = np.sum(
                    [
                        math.pow(self.item_user_train_data[item_j][user] - bias_j, 2)
                        for user in self.item_user_train_data[item_j].keys()
                    ]
                )
                sim_ = 0
                count = 0
                if item_i in self.item_user_train_data:
                    for same_user, score in self.item_user_train_data[item_i].items():
                        norm_i += math.pow(
                            self.item_user_train_data[item_i][same_user] - bias_i, 2
                        )
                        if same_user not in self.item_user_train_data[item_j]:
                            continue
                        count += 1
                        sim_ += (
                            self.item_user_train_data[item_i][same_user] - bias_i
    ...
    def collaborative_filtering_bias(self):
        ...
        for index, row in tqdm(
            enumerate(self.valid_data.values),
            total=len(self.valid_data.values),
            desc="Collaborative Filtering Bias",
        ):
            ...
            bias_i = (
                self.bias["overall_mean"]
                + self.bias["deviation_of_item"][item_i]
            ...
            for i, (item_j, similarity) in enumerate(similar_items):
                ...
                bias_j = (
                    self.bias["overall_mean"]
                    + self.bias["deviation_of_item"][item_j]
                    + self.bias["deviation_of_user"][user]
                )
                rating += similarity * (
                    self.item_user_train_data[item_j][user] - bias_j
                )
                norm += similarity
            if norm != 0:
                rating /= norm
            ...
            if index % 500 == 0 and index != 0:
                logging.info(f"Predicted {index} ratings")
                logging.info(
                    f"RMSE: {calculate_rmse(predicted_ratings, self.valid_data['score'][:index + 1])}"
                )
            if index == len(self.valid_data.values) - 1:
                file_save(
                    self.similarity_map,
                    os.path.join(self.model_directory, "similarity_map.pickle"),
                )
        logging.info(
            f"RMSE: {calculate_rmse(predicted_ratings, self.valid_data['score'])}"
        )

    def predict(self):
        index = 0
        predictions = defaultdict(dict)
        for user, items in tqdm(self.test_data.items(), desc="Prediction"):
            for item_i in items:
                rating = 0
                if item_i in self.bias["deviation_of_item"]:
                    ...
                    rating = (
                        self.bias["overall_mean"] + self.bias["deviation_of_user"][user]
                    )
